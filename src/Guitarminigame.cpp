...
void initGuitar() {
    ...
    g_guitar.currentScale = 0.0f; // to pop up effect
...
void loadGuitarAssets() {
    if (!g_guitar.guitarTexture.loadFromFile("assets/guitarMiniGame/guitar.png"))
        cout << "Guitar texture missing!\n";
    g_guitar.guitarSprite.setTexture(g_guitar.guitarTexture);
    FloatRect gBounds = g_guitar.guitarSprite.getLocalBounds();
    ...
    g_guitar.exitBtn.text.setFillColor(Color::White);

    FloatRect exitTextBounds = g_guitar.exitBtn.text.getLocalBounds();
    ...
    g_guitar.changeModeBtn.text.setFillColor(Color::White);

    FloatRect modeTextBounds = g_guitar.changeModeBtn.text.getLocalBounds();
    ...
    g_guitar.modeText.setFillColor(Color::Yellow);
    ...
    g_guitar.scoreText.setFillColor(Color::Cyan);
    g_guitar.scoreText.setPosition(50.0f, 85.0f); // تحت الـ Mode text بالظبط
}

void setupFretButtons() {
    Vector2f gPos = g_guitar.guitarSprite.getPosition();
    FloatRect gBounds = g_guitar.guitarSprite.getLocalBounds();
    ...
    for (int s = 0; s < MAX_STRINGS; s++) {
        for (int f = 0; f < MAX_FRETS; f++) {
            ...
            b.shape.setFillColor(Color(255, 255, 255, 0)); // شفاف
            float posX = startX + (f * spacingX); float posY = startY + (s * spacingY);
            b.shape.setPosition(posX, posY); b.bounds = b.shape.getGlobalBounds();
            b.buttonText.setFont(g_guitar.uiFont); b.buttonText.setString(to_string(f));
            b.buttonText.setCharacterSize(10); b.buttonText.setFillColor(Color::White);
            FloatRect textBounds = b.buttonText.getLocalBounds();
...
void handleGuitarClick(RenderWindow& window, Vector2i mousePixelPos) {
    ...
    Vector2f mouseViewPos = window.mapPixelToCoords(mousePixelPos, g_guitar.guitarView);
            ...
            if (g_guitar.frets[s][f].bounds.contains(mouseViewPos)) {
                ...
                if (g_guitar.mode == GUITAR_QUEST && g_guitar.questActive) {
                    if (isNoteCorrect(s, f)) {
                        g_guitar.notesPlayedCorrect++; // lw sa7 go the next
                        if (g_guitar.notesPlayedCorrect >= 12) {
...
void updateGuitar(float deltaTime) {
    ...
    if (g_guitar.isAnimating) {
        ...
        if (g_guitar.currentScale >= 1.0f) {
            g_guitar.currentScale = 1.0f; g_guitar.isAnimating = false;
        }
    }
    ...
    if (g_guitar.mode == GUITAR_FREE) {
        ...
        stringstream ss; // like the cout prints anything any data
        int mins = (int)g_guitar.questTimer / 60;
        int secs = (int)g_guitar.questTimer % 60;

        string minsStr = to_string(mins);
        if (mins < 10) {
            minsStr = "0" + minsStr; // if its 5 it will be 05
        }
        ...
        string finalTime = "Mode: Quest | Time: " + minsStr + ":" + secsStr;

        g_guitar.modeText.setString(finalTime);
        string scoreStr = "Notes: " + to_string(g_guitar.notesPlayedCorrect) + " / " + to_string(g_guitar.targetSequenceLength);
...
bool isNoteCorrect(int s, int f) {
    return (g_guitar.targetSequence[g_guitar.notesPlayedCorrect].stringNum == s && g_guitar.targetSequence[g_guitar.notesPlayedCorrect].fretNum == f);
}
