...
bool mapIsWalkable(const GameMap& map, float x, float y, const std::string& mapName) {
    ...
    if (mapName == "leftPassage" || mapName == "rightPassage"|| mapName == "clinic"|| mapName == "sclab"|| mapName == "datacenter"|| mapName == "class7"||mapName =="secur"||mapName == "class8"||mapName =="hallAfter"||mapName =="wcw"||mapName == "wcm1"||mapName=="wcm2"||mapName == "vertPassage") {
     for (const auto& layer : map.layers) {
